...
class AccountWithdrawalServiceTests {
	...
	private final AccountSessionRevoker sessionRevoker =
		org.mockito.Mockito.mock(AccountSessionRevoker.class);
	private final AccountWithdrawalService service =
		new AccountWithdrawalService(repository, sessionRevoker);

	@Test
	void permanentlyDeletesTheCurrentUserAccount() {
		service.withdraw(USER_ID);

		verify(repository).deletePermanently(USER_ID);
		verify(sessionRevoker).revokeAll(USER_ID);
	}
}
